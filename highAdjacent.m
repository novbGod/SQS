...
o = 2;
% while o <= size(a,1)%根据经验，一般满足多紧邻的，最大数字在取一半
...
A = {};A4 = [];Amatrix = [];
for i = 1:8
    A(i,:) = arrayAB_Column(i,11);
end
A = deleteSame(A);
for i = 1:length(A)
    Amatrix(i,:) = A{i};
    A4(i,:) = countAdjacentNbody(A{i},5);
end

figure;
imagesc(A4); % 用颜色表示数值
colorbar;   % 显示颜色条
axis square; % 使坐标轴比例相同，保证正方形
title('Matrix Visualization using imagesc');

% 添加网格线（可选）
hold on;
[x, y] = meshgrid(1:size(A4, 2), 1:size(A4, 1));
% plot(x, y, 'k-', 'LineWidth', 0.5); % 水平线
% plot(y, x, 'k-', 'LineWidth', 0.5); % 垂直线
hold off;
