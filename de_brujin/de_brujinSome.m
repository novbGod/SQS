...
% 调用函数
arrayN3 = gen_debruijn_seqs(2, 3, 1);
arrayNn = gen_debruijn_seqs(k, n, num_to_find);
Judge = zeros(num_to_find,3);
for i = 1:num_to_find
    [Judge(i,1),Judge(i,2),Judge(i,3)] = contains_subarray_periodic(arrayNn(i,:),arrayN3);
end


% 
% % 简单的验证：检查每行是否真的是 De Bruijn 序列
% % (物理直觉：这就好比检查相空间轨迹是否遍历了所有可能的微观状态)
% for i = 1:size(seqs, 1)
%     s = seqs(i, :);
%     % 循环扩展序列以处理周期性边界条件
%     s_cyclic = [s, s(1:n-1)];
% 
%     subsequences = containers.Map('KeyType','char','ValueType','logical');
%     is_valid = true;
% 
%     for j = 1:length(s)
%         sub = num2str(s_cyclic(j : j+n-1));
%         if isKey(subsequences, sub)
%             is_valid = false;
%             break;
%         end
%         subsequences(sub) = true;
%     end
% 
%     if is_valid && subsequences.Count == k^n
%         fprintf('序列 %d 验证通过。\n', i);
%     else
%         fprintf('序列 %d 验证失败！\n', i);
%     end
...
function sequences = gen_debruijn_seqs(k, n, num_req)
