clear;
n = 3;
a = generate_symmetric_debruijn_limited(n,20);
aN3 = generate_symmetric_debruijn_limited(2,1);
%只判断，不删除
% for i = 1:size(a,1)
%     aJudge(i,1) = symmetry01(a(i,:));
%     [NbodyJudge{i,1},NbodyJudge{i,2}] = determineDeBrujin(a(i,:));
% end
...
for i = 1:size(a,1)
    count = count + 1;
    % sym = symmetry01(a(count,:));
    [NbodyJudge,NbodyJudge1] = determineDeBrujin(a(count,:));
    if  NbodyJudge 
        [include{count,1},include{count,2},include{count,3}] = contains_subarray_periodic(a(count,:),aN3);
        if include{count,1} == 1
            C(end+1) = count;
        end
    else
        a(count,:) = [];
