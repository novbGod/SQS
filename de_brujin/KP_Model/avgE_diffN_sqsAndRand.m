...
nums = [100,100,100,100,100,100,100,100];        %计算同一个n的SQS的数量
nmin = 2;
nmax = 9;
StateNums = [20,20,20,150,150,400,400,400,400,400,400,400];%对于每一个SQS，计算量子态的数量
factor = 1;%为了清晰，画方差时乘以一个系数
pt = 18;%字体
...
for n = nmin:nmax
        ...
        if n <= 4
            sqs_seqs = repmat(sqs_seqs,num,1);
        end
    ...
    E0avg(n) = sum(E0sqs(n,:))/size((E0sqs),2);
...
for n = nmin:nmax
    ...
    E0avg(n) = sum(E0rand(n,:))/size((E0rand),2);
    fprintf('average E = %d, n = %d\n', E0avg(n), n);
end
...
E0sqs(1,:) = []; E0rand(1,:) = [];
figure('Color', 'w'); % 创建窗口
plotStatisticalData(E0sqs', E0rand', factor, nmin:nmax);

% 设置坐标轴标签及其字体
xlabel('n (Lattice Size = 2^n)', 'FontName', 'Times New Roman', 'FontSize', pt);
ylabel(sprintf('Ground State Energy (Mean \\pm Std)'), ...
    'FontName', 'Times New Roman', 'FontSize', pt);

% 设置标题及其字体
% title('Ground State Energy Statistical Comparison: SQS vs Random', ...
%     'FontName', 'Times New Roman', 'FontSize', pt, 'FontWeight', 'bold');

% 设置图例及其字体
lgd = legend({'SQS', 'Random structure'}, 'Location', 'best');
set(lgd, 'FontName', 'Times New Roman', 'FontSize', pt);
legend('boxoff');

% --- 关键：设置坐标轴刻度字体 ---
% 这步会将坐标轴上的数字也改为 Times New Roman
set(gca, 'FontName', 'Times New Roman', 'FontSize', pt);
